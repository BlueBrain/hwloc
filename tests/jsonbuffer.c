...
int main(void)
{
  ...
  hwloc_topology_export_jsonbuffer(topology, &buf1, &size1);
  hwloc_topology_destroy(topology);
  printf("exported to buffer %p length %d\n", buf1, size1);

  hwloc_topology_init(&topology);
  hwloc_topology_set_jsonbuffer(topology, buf1, size1);
  hwloc_topology_load(topology);
  assert(!hwloc_topology_is_thissystem(topology));
  hwloc_topology_export_jsonbuffer(topology, &buf2, &size2);
  hwloc_topology_destroy(topology);
