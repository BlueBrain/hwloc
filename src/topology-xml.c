			   ...
			   hwloc__xml_import_state_t state)
{
  ...
  if (nb && name) {
	...
	goto out_free;
      ...
      while (1) {
	char *attrname, *attrvalue;
	if (state->next_attr(&childstate, &attrname, &attrvalue) < 0)
	  break;
...
hwloc__xml_export_object (hwloc__xml_export_output_t output, hwloc_topology_t topology, hwloc_obj_t obj)
{
  ...
  for(i=0; i<obj->valarray_count; i++) {
    ...
    output->new_child(output, "valarray");
    sprintf(tmp, "%u", nb);
    output->new_prop(output, "nb", tmp);
    output->new_prop(output, "name", name);
    output->end_props(output, nb);
    for(j=0; j<nb; j++) {
      output->new_child(output, "valarrayslot");
      sprintf(tmp, "%f", obj->valarray[i]->values[j]);
      output->new_prop(output, "value", tmp);
      sprintf(tmp, "%u", obj->valarray[i]->idx[j]);
      output->new_prop(output, "idx", tmp);
      output->end_props(output, 0);
      output->end_child(output, "valarrayslot", 0);
    }
    output->end_child(output, "valarray", nb);
    free(name);
  }
