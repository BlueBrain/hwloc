...
void hwloc_look_libpci(struct hwloc_topology *topology);
...
#ifdef HWLOC_LINUX_SYS
  topology->base_backends->backend->hwloc_linuxfs_pci_lookup_osdevices(topology, pcidev);
#endif
}
...
#ifdef HWLOC_LINUX_SYS
	  err = topology->base_backends->backend->hwloc_linuxfs_get_pcidev_cpuset(topology, hostbridge->first_child, cpuset);
	  err = -1;
#else
    err = -1;
#endif
...
hwloc_get_backend(void){
	...
	backend->name = "libpci";
	backend->hwloc_look = hwloc_look_libpci;
	backend->hwloc_set_hooks = NULL;
	backend->hwloc_backend_init = NULL;
	backend->hwloc_backend_exit = NULL;
